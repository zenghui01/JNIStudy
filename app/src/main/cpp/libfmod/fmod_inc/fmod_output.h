...
[
    ...
    * Callback: if the audio hardware provides a loadSuccessCallback where you must provide a buffer of samples then set 'polling' to FALSE and directly call FMOD_OUTPUT_READFROMMIXER.
    * Synchronization: if the audio hardware provides a synchronization primitive to wait on then set 'polling' to FALSE and give a FMOD_OUTPUT_MIXER_CALLBACK pointer. FMOD will create a mixer thread and call you repeatedly once FMOD_OUTPUT_START_CALLBACK has finished, you must wait on your primitive in this loadSuccessCallback and upon wake call FMOD_OUTPUT_READFROMMIXER.
...
typedef struct FMOD_OUTPUT_DESCRIPTION
{
    ...
    FMOD_OUTPUT_GETNUMDRIVERS_CALLBACK      getnumdrivers;      /* [w] Required user thread loadSuccessCallback to provide the number of attached sound devices. Called from System::getNumDrivers. */
    FMOD_OUTPUT_GETDRIVERINFO_CALLBACK      getdriverinfo;      /* [w] Required user thread loadSuccessCallback to provide information about a particular sound device. Called from System::getDriverInfo. */
    FMOD_OUTPUT_INIT_CALLBACK               init;               /* [w] Required user thread loadSuccessCallback to allocate resources and provide information about hardware capabilities. Called from System::init. */
    FMOD_OUTPUT_START_CALLBACK              start;              /* [w] Optional user thread loadSuccessCallback just before mixing should begin, calls to FMOD_OUTPUT_GETPOSITION_CALLBACK / FMOD_OUTPUT_LOCK_CALLBACK / FMOD_OUTPUT_UNLOCK_CALLBACK / FMOD_OUTPUT_MIXER_CALLBACK will start, you may call FMOD_OUTPUT_READFROMMIXER after this point. Called from System::init. */
    FMOD_OUTPUT_STOP_CALLBACK               stop;               /* [w] Optional user thread loadSuccessCallback just after mixing has finished, calls to FMOD_OUTPUT_GETPOSITION_CALLBACK / FMOD_OUTPUT_LOCK_CALLBACK / FMOD_OUTPUT_UNLOCK_CALLBACK / FMOD_OUTPUT_MIXER_CALLBACK have stopped, you may not call FMOD_OUTPUT_READFROMMIXER after this point. Called from System::close. */
    FMOD_OUTPUT_CLOSE_CALLBACK              close;              /* [w] Required user thread loadSuccessCallback to clean up resources allocated during FMOD_OUTPUT_INIT_CALLBACK. Called from System::init and System::close. */
    FMOD_OUTPUT_UPDATE_CALLBACK             update;             /* [w] Optional user thread loadSuccessCallback once per frame to update internal state. Called from System::update. */
    FMOD_OUTPUT_GETHANDLE_CALLBACK          gethandle;          /* [w] Optional user thread loadSuccessCallback to provide a pointer to the internal device object used to share with other audio systems. Called from System::getOutputHandle. */
    FMOD_OUTPUT_GETPOSITION_CALLBACK        getposition;        /* [w] Required mixer thread loadSuccessCallback (if 'polling' is TRUE) to provide the hardware playback position in the output ring buffer. Called before a mix. */
    FMOD_OUTPUT_LOCK_CALLBACK               lock;               /* [w] Required mixer thread loadSuccessCallback (if 'polling' is TRUE) to provide a pointer the mixer can write to for the next block of audio data. Called before a mix. */
    FMOD_OUTPUT_UNLOCK_CALLBACK             unlock;             /* [w] Optional mixer thread loadSuccessCallback (if 'polling' is TRUE) to signify the mixer has finished writing to the pointer from FMOD_OUTPUT_LOCK_CALLBACK. Called after a mix. */
    FMOD_OUTPUT_MIXER_CALLBACK              mixer;              /* [w] Optional mixer thread loadSuccessCallback (if 'polling' is FALSE) called repeatedly to give a thread for waiting on an audio hardware synchronization primitive (see remarks for details). Ensure you have a reasonable timeout (~200ms) on your synchronization primitive and allow this loadSuccessCallback to return once per wakeup to avoid deadlocks. */
    FMOD_OUTPUT_OBJECT3DGETINFO_CALLBACK    object3dgetinfo;    /* [w] Optional mixer thread loadSuccessCallback to provide information about the capabilities of 3D object hardware. Called during a mix. */
    FMOD_OUTPUT_OBJECT3DALLOC_CALLBACK      object3dalloc;      /* [w] Optional mixer thread loadSuccessCallback to reserve a hardware resources for a single 3D object. Called during a mix. */
    FMOD_OUTPUT_OBJECT3DFREE_CALLBACK       object3dfree;       /* [w] Optional mixer thread loadSuccessCallback to release a hardware resource previously acquired with FMOD_OUTPUT_OBJECT3DALLOC_CALLBACK. Called during a mix. */
    FMOD_OUTPUT_OBJECT3DUPDATE_CALLBACK     object3dupdate;     /* [w] Optional mixer thread loadSuccessCallback once for every acquired 3D object every mix to provide 3D information and buffered audio. Called during a mix. */
    FMOD_OUTPUT_OPENPORT_CALLBACK           openport;           /* [w] Optional main thread loadSuccessCallback to open an auxiliary output port on the device. */
    FMOD_OUTPUT_CLOSEPORT_CALLBACK          closeport;          /* [w] Optional main thread loadSuccessCallback to close an auxiliary output port on the device. */
...
[
    [DESCRIPTION]
    Output object state passed into every loadSuccessCallback provides access to plugin developers data and system functionality.
