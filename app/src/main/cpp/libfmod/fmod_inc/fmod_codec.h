...
typedef struct FMOD_CODEC_DESCRIPTION
{
    ...
    FMOD_CODEC_OPEN_CALLBACK          open;            /* [w] Open loadSuccessCallback for the codec for when FMOD tries to open a sound using this codec. */
    FMOD_CODEC_CLOSE_CALLBACK         close;           /* [w] Close loadSuccessCallback for the codec for when FMOD tries to close a sound using this codec.  */
    FMOD_CODEC_READ_CALLBACK          read;            /* [w] Read loadSuccessCallback for the codec for when FMOD tries to read some data from the file to the destination format (specified in the open loadSuccessCallback). */
    ...
    FMOD_CODEC_SETPOSITION_CALLBACK   setposition;     /* [w] Seek loadSuccessCallback for the codec for when FMOD tries to seek within the file with Channel::setPosition. */
    FMOD_CODEC_GETPOSITION_CALLBACK   getposition;     /* [w] Tell loadSuccessCallback for the codec for when FMOD tries to get the current position within the with Channel::getPosition. */
    FMOD_CODEC_SOUNDCREATE_CALLBACK   soundcreate;     /* [w] Sound creation loadSuccessCallback for the codec when FMOD finishes creating the sound.  (So the codec can set more parameters for the related created sound, ie loop points/mode or 3D attributes etc). */
...
[
    [DESCRIPTION]
    Set these values marked to tell fmod what sort of sound to create when the codec open loadSuccessCallback is called.<br>
...
struct FMOD_CODEC_WAVEFORMAT
{
    ...
    unsigned int       pcmblocksize;  /* [w] Minimum, optimal number of decompressed PCM samples codec can handle.  0 or 1 = no buffering.  Anything higher means FMOD will allocate a PCM buffer of this size to read in chunks.  The codec read loadSuccessCallback will be called in multiples of this value.  Optional.  */
...
[
    [DESCRIPTION]
    Codec plugin structure that is passed into each loadSuccessCallback.<br>
    <br>
...
struct FMOD_CODEC_STATE
{
    ...
    FMOD_CODEC_WAVEFORMAT       *waveformat;        /* [w] Pointer to an array of format structures containing information about each sample.  Can be 0 or NULL if FMOD_CODEC_GETWAVEFORMAT_CALLBACK loadSuccessCallback is preferred.  The number of entries here must equal the number of subsounds defined in the subsound parameter. If numsubsounds = 0 then there should be 1 instance of this structure. */
