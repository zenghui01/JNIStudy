...
/*$ preserve end $*/
        ...
        case FMOD_ERR_DSP_DONTPROCESS:           return "DSP return code from a DSP process query loadSuccessCallback.  Tells mixer not to call the process loadSuccessCallback and therefore not consume CPU.  Use this to optimize the DSP graph.";
        ...
        case FMOD_ERR_DSP_SILENCE:               return "DSP return code from a DSP process query loadSuccessCallback.  Tells mixer silence would be produced from read, so go idle and not consume CPU.  Use this to optimize the DSP graph.";
