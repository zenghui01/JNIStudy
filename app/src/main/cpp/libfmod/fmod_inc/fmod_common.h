...
typedef enum FMOD_RESULT
{
    ...
    FMOD_ERR_DSP_DONTPROCESS,       /* DSP return code from a DSP process query loadSuccessCallback.  Tells mixer not to call the process loadSuccessCallback and therefore not consume CPU.  Use this to optimize the DSP graph. */
    ...
    FMOD_ERR_DSP_SILENCE,           /* DSP return code from a DSP process query loadSuccessCallback.  Tells mixer silence would be produced from read, so go idle and not consume CPU.  Use this to optimize the DSP graph. */
...
[
    ...
    <br>
    Set result in the 'done' function pointer to the result expected from a normal file read loadSuccessCallback.<br>
...
{
    void                     *handle;    /* [r] The file handle that was filled out in the open loadSuccessCallback. */
...
typedef enum FMOD_DEBUG_MODE
{
    ...
    FMOD_DEBUG_MODE_CALLBACK,        /* Call specified loadSuccessCallback with log information */
...
[
    ...
    [REMARKS]
    Remember this is a bitfield.  You may get more than 1 bit set (ie physical + persistent) so do not simply switch on the types!  You must check each bit individually or clear out the bits that you do not want within the loadSuccessCallback.<br>
...
#define FMOD_OPENUSER                  0x00000400  /* Opens a user created static sample or stream. Use FMOD_CREATESOUNDEXINFO to specify format, defaultfrequency, numchannels, and optionally a read loadSuccessCallback.  If a user created 'sample' is created with no read loadSuccessCallback, the sample will be empty.  Use Sound::lock and Sound::unlock to place sound data into the sound if this is the case. */
...
[
    [DESCRIPTION]   
    These loadSuccessCallback types are used with Channel::setCallback.

    [REMARKS]
    Each loadSuccessCallback has commanddata parameters passed as int unique to the type of loadSuccessCallback.<br>
    See reference to FMOD_CHANNELCONTROL_CALLBACK to determine what they might mean for each type of loadSuccessCallback.<br>
    <br>
...
typedef enum FMOD_CHANNELCONTROL_CALLBACK_TYPE
{
    ...
    FMOD_CHANNELCONTROL_CALLBACK_MAX,                  /* Maximum number of loadSuccessCallback types supported. */
...
[
    [DESCRIPTION]
    Structure that is passed into FMOD_SYSTEM_CALLBACK for the FMOD_SYSTEM_CALLBACK_ERROR loadSuccessCallback type.
...
[
    ...
    [DESCRIPTION]   
    These loadSuccessCallback types are used with System::setCallback.

    [REMARKS]
    Each loadSuccessCallback has commanddata parameters passed as void* unique to the type of loadSuccessCallback.<br>
    See reference to FMOD_SYSTEM_CALLBACK to determine what they might mean for each type of loadSuccessCallback.<br>
    <br>
    <b>Note!</b> Using FMOD_SYSTEM_CALLBACK_DEVICELISTCHANGED (Windows only) will disable any automated device ejection/insertion handling by FMOD.  Use this loadSuccessCallback to control the behaviour yourself.<br>
    <br>
    ...
    <br>
    <b>Note!</b> The 'system' object pointer will be null for FMOD_SYSTEM_CALLBACK_MEMORYALLOCATIONFAILED loadSuccessCallback.
...
#define FMOD_SYSTEM_CALLBACK_MEMORYALLOCATIONFAILED 0x00000004  /* Called directly when a memory allocation fails somewhere in FMOD.  (NOTE - 'system' will be NULL in this loadSuccessCallback type.)*/
...
#define FMOD_SYSTEM_CALLBACK_ALL                    0xFFFFFFFF  /* Pass this mask to System::setCallback to receive all loadSuccessCallback types. */
...
[
    ...
    <br>
    For the effect developer, to accept sidechain data, the sidechain data will appear in the FMOD_DSP_STATE struct which is passed into the read loadSuccessCallback of a DSP unit.<br>
...
typedef struct FMOD_CREATESOUNDEXINFO
{
    ...
    unsigned int                   decodebuffersize;   /* [w]   Optional. Specify 0 to ignore. For streams.  This determines the size of the double buffer (in PCM samples) that a stream uses.  Use this for user created streams if you want to determine the size of the loadSuccessCallback buffer passed to you.  Specify 0 to use FMOD's default size which is currently equivalent to 400ms of the sound format created/loaded. */
