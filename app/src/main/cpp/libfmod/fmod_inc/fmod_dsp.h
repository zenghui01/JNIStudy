...
[
    ...
    [REMARKS]
    A process loadSuccessCallback will be called twice per mix for a DSP unit.  Once with the FMOD_DSP_PROCESS_QUERY command, then conditionally, FMOD_DSP_PROCESS_PERFORM.<br>
...
typedef struct FMOD_DSP_DESCRIPTION
{
    ...
    FMOD_DSP_CREATE_CALLBACK            create;             /* [w] Create loadSuccessCallback.  This is called when DSP unit is created.  Can be null. */
    FMOD_DSP_RELEASE_CALLBACK           release;            /* [w] Release loadSuccessCallback.  This is called just before the unit is freed so the user can do any cleanup needed for the unit.  Can be null. */
    FMOD_DSP_RESET_CALLBACK             reset;              /* [w] Reset loadSuccessCallback.  This is called by the user to reset any history buffers that may need resetting for a filter, when it is to be used or re-used for the first time to its initial clean state.  Use to avoid clicks or artifacts. */
    FMOD_DSP_READ_CALLBACK              read;               /* [w] Read loadSuccessCallback.  Processing is done here.  Can be null. */
    FMOD_DSP_PROCESS_CALLBACK           process;            /* [w] Process loadSuccessCallback.  Can be specified instead of the read loadSuccessCallback if any channel format changes occur between input and output.  This also replaces shouldiprocess and should return an error if the effect is to be bypassed.  Can be null. */
    FMOD_DSP_SETPOSITION_CALLBACK       setposition;        /* [w] Set position loadSuccessCallback.  This is called if the unit wants to update its position info but not process data, or reset a cursor position internally if it is reading data from a certain source.  Can be null. */
    ...
    FMOD_DSP_SYSTEM_REGISTER_CALLBACK   sys_register;       /* [w] Register loadSuccessCallback.  This is called when DSP unit is loaded/registered.  Useful for 'global'/per system object init for plugin.  Can be null. */
    FMOD_DSP_SYSTEM_DEREGISTER_CALLBACK sys_deregister;     /* [w] Deregister loadSuccessCallback.  This is called when DSP unit is unloaded/deregistered.  Useful as 'global'/per system object shutdown for plugin.  Can be null. */
    FMOD_DSP_SYSTEM_MIX_CALLBACK        sys_mix;            /* [w] System mix stage loadSuccessCallback.  This is called when the mixer starts to execute or is just finishing executing.  Useful for 'global'/per system object once a mix update calls for a plugin.  Can be null. */
...
[
    [DESCRIPTION]
    DSP plugin structure that is passed into each loadSuccessCallback.
