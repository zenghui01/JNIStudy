...
import com.testndk.jnistudy.utils.interfaces.IPanelHeightTarget;
...
public class KeyboardUtil {
    ...
    public static ViewTreeObserver.OnGlobalLayoutListener attach(final Activity activity, IPanelHeightTarget target,
        ...
        final boolean isTranslucentStatus = ViewUtil.isTranslucentStatusForKeyboardUtil(activity);
        ...
        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN) {
            // http://jira.miaozhang.cn/browse/BX2018-1504
            // 为解决特殊机型获取的 screen height 不包含 status bar 的高度
//            if (isSpecialDevice()) {
//                screenHeight = contentView.getHeight() - contentView.getPaddingTop();
//            } else {
...
//            }
    ...
    private static boolean isSpecialDevice() {
        ...
        // android P 版本的 Oneplus 6， Pixel 上 screen height 单独处理
        if ((("ONEPLUS A6000".equals(Build.MODEL)) || ("Pixel".equals(Build.MODEL))) && (Build.VERSION.SDK_INT == 28/*Android P*/)) {
    ...
    public static ViewTreeObserver.OnGlobalLayoutListener attach(final Activity activity, OnKeyboardShowingListener listener) {
        return attach(activity, null, listener);
    }
    ...
    private static class KeyboardStatusListener implements ViewTreeObserver.OnGlobalLayoutListener {
        ...
        public void onGlobalLayout() {
            ...
            if (isTranslucentStatus) {
                ...
                if (!isOverlayLayoutDisplayHContainStatusBar) {
                    ...
                    isOverlayLayoutDisplayHContainStatusBar = false;
                }
        ...
        private void calculateKeyboardHeight(final int displayHeight) {
            // first result.
            if (panelHeightTarget != null && previousDisplayHeight == 0) {
                previousDisplayHeight = displayHeight;
            ...
            if (KPSwitchConflictUtil.isHandleByPlaceholder(isFullScreen, isTranslucentStatus,
                ...
                //Log.d(TAG, String.format("action bar over layout %d display height: %d",((View) contentView.getParent()).getHeight(), displayHeight));
            ...
            if (changed) {
                ...
                if (this.panelHeightTarget != null && this.panelHeightTarget.getHeight() != validPanelHeight) {
        ...
        private void calculateKeyboardShowing(final int displayHeight) {
            ...
            Log.e(TAG, "calculateKeyboardShowing() lastKeyboardShowing:" + lastKeyboardShowing + ", isKeyboardShowing:" + isKeyboardShowing);
